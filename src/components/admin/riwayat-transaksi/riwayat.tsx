...
function CheckTable() {
  ...
  const filteredData = useMemo(() => {
    return data.filter((row) => {
      ...
      const matchDate = startDate
        ? new Date(row.created_at).getTime() >= new Date(startDate).getTime()
        : true;
  ...
  return (
   ...
   <header className="relative flex flex-col gap-y-4 md:flex-row md:items-center md:justify-between pt-4">
  ...
  <div className="w-full md:w-auto">
    <button
      ...
      className="w-full md:w-auto rounded bg-[#294B29] px-4 py-2 font-bold text-white hover:bg-green-800 active:bg-[#294B29] transition-colors"
    >
      Download Data
    </button>
  </div>
